...
function createTable(records) {
    ...
    for (var i = 0; i < TaxonomyFile.length; i++) {
        ...
        try {
            ...
            if (TaxonomyFile[i][8].indexOf(records[0].data.value) >= 0) {
                var rowPos = i
    ...
    for (var iter = 0; iter < RecordRow[6].length; iter++) {
        ...
        try {
            ...
            var cssBPMN, cssPDF, cssLiveVideo, cssAnnotVideo, path = RecordRow[8][iter]
            ...
            tableStructure += "<tr>" +
                ...
                // '&emsp;<img src="../assets/Images/video-annotate.svg" id="videoViewer' + (iter + 1) + '" style="color: black;padding: 3px;height:32px;' + cssAnnotVideo + '" title="Annotated video" alt="videoViewer" onclick="VideoAnnotation(' + "'" + escape(RecordRow[8][iter]) + "'" + ')">' +
...
function hierarchy(Toxonomy_File) {
    $("#diagram").html("")
